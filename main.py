...
# MBTI와 추천 직업 + 설명 데이터 (16유형 완성)
mbti_jobs = {
    "ISTJ": [
        ("회계사", "정확성과 꼼꼼함을 바탕으로 재무와 회계를 관리하는 전문가"),
        ("데이터 분석가", "데이터를 분석해 인사이트를 도출하는 직업"),
        ("공무원", "공공 서비스를 제공하며 안정적인 환경에서 일하는 직업"),
        ("품질관리자", "제품이나 서비스의 품질을 관리하고 개선하는 전문가")
    ],
    "ISFJ": [
    ...
    "INTJ": [
        ...
        ("변호사", "법률 지식을 바탕으로 사건을 해결하는 전문가")
    ],
    "ISTP": [
        ("엔지니어", "기계, 전자, 건축 등 기술 분야에서 일하는 전문가"),
        ("경찰관", "사회 질서를 유지하고 안전을 지키는 직업"),
        ("항공 정비사", "항공기의 안전 운항을 위해 점검과 수리를 하는 직업"),
